        ...
        <div class="container my-4">
            <div class="card">
                <div class="card-header text-white">
                    <h2 class="text-center text-light">{{__('inventory.edit_inventory')}}</h2>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('inventario.update', $inventario->id) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="product_id" class="form-control" name="product_id" required>
                                @foreach($productos as $producto)
                                    <option value="{{ $producto->id }}" {{ $producto->id == $inventario->product_id ? 'selected' : '' }}>
                                        {{ $producto->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <x-text-input id="stock" class="form-control" type="number" name="stock" value="{{ $inventario->stock }}" required />
